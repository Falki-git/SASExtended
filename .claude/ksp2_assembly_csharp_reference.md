...
`architectural_patterns_ksp2.md`; this file is the "where does X live" index. Mod-agnostic;
for the current mod see `mod_specifics.md`.

## Facts

- **File:** `Packages/KSP2_x64/Assembly-CSharp.dll` (~14 MB)
...
- **Publicization:** the shipped copy keeps original access modifiers (private/internal).
  Do **not** run `publicize.bat` (it's the user's choice and currently broken) — reach
  non-public members via reflection instead. See CLAUDE.md → "Private game members".
...
The Redux build layers extra systems on top of stock KSP2:
...
- **`Redux.Modules`** — new part modules following the dual pattern:
  ...
  Study these as reference implementations before writing a new module.
- **`Redux.WorldVis`** — `PqsOverlay`, `PqsVisSetup` — **planet-surface overlay rendering**;
  precedent for drawing a data overlay onto a body.
- **`Redux.Ksp1Import.*`** — importer that maps KSP1-style part configs/modules into KSP2
  (`Ksp1PartModuleImporterRegistry`, `Ksp1PartComponentModule_*`). Useful when porting a
  KSP1 / SpaceWarp-1.x era mod.
...
| `CelestialBodyComponent` | `KSP.Sim.impl` | A celestial body (radius, name, …). |
...
| `PartComponentModule` | `KSP.Sim.impl` | Base for sim-side module logic. |
| `PartBehaviourModule` / `ModuleData` | `KSP.Sim.Definitions` | Base for view-side + serialized data. |
| `ResourceFlowRequestBroker`, `ResourceContainerGroup`, `ResourceDefinitionDatabase` | `KSP.Sim.ResourceSystem` / `KSP.Game` | Resource consumption/production. |
| `MapProvider`, `KSP.Map.*` | `KSP.Game` / `KSP.Map` | Map view + overlay placement. |
| `Redux.Modules.PartComponentModule_ResourceScanner` | `Redux.Modules` | Reference module implementation. |
| `Redux.WorldVis.PqsOverlay` | `Redux.WorldVis` | Surface overlay precedent. |
| Messages: `GameStateChangedMessage`, `VesselChangedMessage`, `MapViewEnteredMessage` | `KSP.Messages` | Lifecycle hooks. |
